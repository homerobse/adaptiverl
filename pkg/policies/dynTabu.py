...
class DynTabu(object):
   '''
   Dynamic Tabu policy
   DESCRIPTION: Tabu policy with an adaptable variance of the noise (sigma)
   '''
   ...
   def decide(self):
      if len(self.agent.actions)==0: #first action
         ...
         xNeighborhood = x_allowed[ngbhdAllowed]
         self.sigma=abs(max(xNeighborhood)-min(xNeighborhood))
